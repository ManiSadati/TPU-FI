...
# Load the CSV file
df = pd.read_csv('segmentation_fi_results.csv', sep='\t')
...
df['fit_times_avf_critical'] = df['sdc_rate'] * df['layer_vs_fault_fit_rate']

# Step 3: Save the enriched data to a new file
df.to_csv('segmentation_fi_results_with_fit.csv', sep='\t', index=False)
...
sdc_per_type = df.groupby('type').apply(lambda x: x['sdc_count'].sum() / x['total runs'].sum()).reset_index()
sdc_per_type.columns = ['type', 'sdc_rate_total']

# Step 7: Compute SDC rate per layer type
sdc_per_layer = df.groupby('name').apply(lambda x: x['sdc_count'].sum() / x['total runs'].sum()).reset_index()
